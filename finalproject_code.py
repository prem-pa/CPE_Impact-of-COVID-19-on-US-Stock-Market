...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 20 16:45:45 2020

@author: prem
"""

# api_key = zk9hWP522GvyDxfggGgNGdIyEoQZPRl2Teaj48SvsZ9ciUAyCN1YwdR1jbnY
# api_key1 = 5XBMT53K2J7WAHGQ
...
#The following three lines of code will take time to execute. Run them individually
confirmed_cases = pd.read_csv(csv_url_confirmed_cases)
recoved_cases = pd.read_csv(csv_url_recovered_cases)
deaths = pd.read_csv(csv_url_deaths)

#Saving the csv files locally
confirmed_cases.to_csv('G:/My Drive/Semester 2/CPE 695/Final Project/Data files/confirmed_cases_' + datetoday + '.csv', index=False, line_terminator='\n')
recoved_cases.to_csv('G:/My Drive/Semester 2/CPE 695/Final Project/Data files/recoved_cases_' + datetoday + '.csv', index=False, line_terminator='\n')
deaths.to_csv('G:/My Drive/Semester 2/CPE 695/Final Project/Data files/deaths_' + datetoday + '.csv', index=False, line_terminator='\n')

#Data cleaning
confirmed_cases = confirmed_cases[confirmed_cases['Country/Region'] == 'US']
recoved_cases = recoved_cases[recoved_cases['Country/Region'] == 'US']
deaths = deaths[deaths['Country/Region'] == 'US']

confirmed_cases = confirmed_cases.filter(['Date', 'Value'])
recoved_cases = recoved_cases.filter(['Date', 'Value'])
deaths = deaths.filter(['Date', 'Value'])

#Renaming columns
confirmed_cases = confirmed_cases.rename({"Value":"Confirmed_cases"}, axis='columns')
recoved_cases = recoved_cases.rename({"Value":"Recoved_cases"}, axis='columns')
deaths = deaths.rename({"Value":"Deaths"}, axis='columns')

#Data Merging (COVID)
covid_data = pd.merge(confirmed_cases, recoved_cases, on=['Date'], how='outer')
covid_data_final = pd.merge(covid_data, deaths, on=['Date'], how='outer')

#Stock market data
import yfinance as yf
#test = yf.download('AAP','2018-01-01','2020-04-04')

#Loading compnay index data
company_data = pd.read_csv('G:/My Drive/Semester 2/CPE 695/Final Project/Data files/constituents_csv.csv')
